...
export default async function ItemDetailPage({
  ...
  const specs = [
    { icon: Tag, label: t("detail.category"), value: t(`itemCategory.${item.itemCategory}`) },
    { icon: ShoppingBag, label: t("detail.condition"), value: t(`condition.${item.condition}`) },
    {
      icon: MapPin,
      label: t("detail.location"),
      value: `${item.city}, ${t(`country.${item.country}`)}`,
    },
  ];
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      <nav className="mb-5 flex items-center gap-1 text-sm text-neutral-500">
        <Link href="/achat-vente" className="hover:text-brand-700">
          {t("category")}
        </Link>
          ...
          <div className="sticky top-20 space-y-4">
            ...
            <div className="card p-5">
              ...
              <div className="mt-4 space-y-2">
                <a href={`tel:${item.contactPhone}`} className="btn-primary w-full">
                  <Phone size={16} />
                  {t("detail.call")}
                </a>
                <a href={`mailto:${item.contactEmail}`} className="btn-secondary w-full">
                  <Mail size={16} />
                  {t("detail.email")}
                </a>
              </div>
